...
function from7(tokenIdOrAddress) {
...
function encode2(hrp, data, options = {}) {
...
function format(address, options = {}) {
  ...
  const zone = zoneId != null ? toBytes2(zoneId) : new Uint8Array();
  const data = concat(version, zone, fromHex(address));
  return encode2(hrp, data);
}
function parse(tempoAddress) {
  let hrp;
  let data;
  try {
    const decoded = decode(tempoAddress);
    hrp = decoded.hrp;
    data = decoded.data;
  } catch {
    throw new InvalidChecksumError2({ address: tempoAddress });
  }
  ...
  if (data.length < 1 || data[0] !== 0)
    throw new InvalidVersionError({
      ...
      version: data.length > 0 ? data[0] : void 0
    });
  const payload = data.slice(1);
  let zoneId;
  let rawAddress;
  if (hrp === "tempoz") {
    const { value, size: size2 } = fromBytes2(payload);
    zoneId = value;
    rawAddress = payload.slice(size2);
  } else {
    zoneId = void 0;
    rawAddress = payload;
  }
  ...
  const address = checksum(fromBytes(rawAddress));
...
var InvalidVersionError = class extends BaseError2 {
  constructor({ address, version }) {
    super(`Tempo address "${address}" has unsupported version ${version === void 0 ? "(missing)" : `0x${version.toString(16).padStart(2, "0")}`}. Only version 0x00 is supported.`);
    Object.defineProperty(this, "name", {
      enumerable: true,
      configurable: true,
      writable: true,
  ...
  if (!transaction.from && signatureEnvelope) {
    try {
      transaction.from = extractAddress({
        payload: getSignPayload3(from9(transaction)),
        signature: signatureEnvelope,
        root: true
      });
    } catch {
    ...
    if (options.format === "feePayer" && signature) {
      ...
      if (sig.type === "secp256k1")
        return recoverAddress({
          payload: getSignPayload3(from9(envelope)),
...
//# sourceMappingURL=chunk-XRHIFBPN.js.map
